...
public class StudentsController implements Initializable {
   ...
   @Override
    public void initialize(URL url, ResourceBundle rb) {
       ...
       //selectedSubjectViewModel = new SubjectViewModel(subject);
       selectedStudentViewModel = new StudentViewModel(student);
                ...
                if (change.next()) { //provjeri i udi u cursor koji radi promjenu
                    if (change.wasRemoved()) {
                        change.getRemoved().forEach(suvm -> {
                            try {
                                RepositoryFactory.getRepository().deleteSubject(suvm.getSubject());
                            } catch (Exception ex) {
                                Logger.getLogger(StudentsController.class.getName()).log(Level.SEVERE, null, ex);
                            }
                        });
                        ...
                        change.getAddedSubList().forEach(suvm ->{
                            try {
                                int idSubject = RepositoryFactory.getRepository().addSubject(suvm.getSubject());
